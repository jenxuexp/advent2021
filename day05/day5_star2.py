...
def parse_rules(fname):
    ...
    for line in open(fname):
        ...
        if x1 == x2:
            ...
            print('Recorded as right diagonal')
            diag_right_list.append([min(y1, y2), min(x1, x2), abs(y2 - y1) + 1])
        elif (x2 - x1) == -(y2 - y1):
            print('Recorded as left diagonal')
            diag_left_list.append([min(y1, y2), min(x1, x2), abs(y2 - y1) + 1])
    return vertical_list, horizontal_list, diag_left_list, diag_right_list, max_x, max_y
...
def main():
    vert_list, hor_list, dl_list, dr_list, max_x, max_y = parse_rules('day5_input.txt')
    ...
    for cl in hor_list:
        ...
        print(tracking_array)
    for cl in dl_list:
        print('Left diagonal line', cl)
        add_mat = np.fliplr(np.eye(cl[2]))
        tracking_array[cl[0]:(cl[0]+cl[2]), cl[1]:(cl[1]+cl[2])] += add_mat
        print(tracking_array)
    for cl in dr_list:
        print('Right diagonal line', cl)
        add_mat = np.eye(cl[2])
        tracking_array[cl[0]:(cl[0]+cl[2]), cl[1]:(cl[1]+cl[2])] += add_mat
        print(tracking_array)
    print('Final Score: ', np.sum(tracking_array.flatten()>1))
